package com.software.tongji.easygo.AchievementModule.MapView;
...
import com.software.tongji.easygo.Bean.Lasso;
...
public class MyMapView extends View {
    ...
    private static final String LOCKED_PROVINCE_COLOR = "#DCDCDC";
    ...
    public void setOnChoseProvince(OnProvinceClickListener listener){
    ...
    private void drawMap(Canvas canvas) {
        ...
        if (map.getProvinceList().size()>0){
            int b=-1;
            for (int i=0;i<map.getProvinceList().size();i++){
                //保证最后画选中的省份，防止黑色边框线被灰色线覆盖
                if (map.getProvinceList().get(i).getLineColor()==Color.parseColor(SELECTED_PROVINCE_LINE_COLOR)){
                    b=i;
                }else{
                    setPaintProvinceColor(paint, i);
                    linepaint.setColor(map.getProvinceList().get(i).getLineColor());
                    for (Path p:map.getProvinceList().get(i).getPathList()){
            ...
            if(b>-1){
                setPaintProvinceColor(paint, b);
                linepaint.setColor(map.getProvinceList().get(b).getLineColor());
                for (Path p:map.getProvinceList().get(b).getPathList()){
                    canvas.drawPath(p, paint);
                    canvas.drawPath(p, linepaint);
                }
            }
    ...
    void setPaintProvinceColor(Paint paint,int index){
        Province province = map.getProvinceList().get(index);
        if(province.isLocked()){
            paint.setColor(Color.parseColor(LOCKED_PROVINCE_COLOR));
        }else{
...
//         将变成(  x0-(x0-x)*s,  y0-(y0-y)*s  )
    ...
    private void scalePoints(Canvas canvas, float scale) {
        ...
        for (Province province:map.getProvinceList()){
            paint.setColor(province.getColor());
            List<Lasso> listLasso=new ArrayList<>();
            List<Path> pathList=new ArrayList<>();
            for (Path p:province.getPathList()){
                //遍历Path中的所有点，重置点的坐标
            ...
            province.setPathList(pathList);
            //拿到path转换之后的Lasso对象，用来判断点击的是哪个省份,即判断点是否在path画出的区域内
    ...
    private void changeProvinceColor(MotionEvent event, RectF rectF) {
        for (Province province:map.getProvinceList()){
            province.setLineColor(Color.parseColor(UNSELECTED_PROVINCE_LINE_COLOR));
        }
        for (Province p:map.getProvinceList()){
            for (Lasso lasso:p.getPathLasso()){
                ...
                if (lasso.contains(pf.x,pf.y)){
                    ...
                    p.setLineColor(Color.parseColor(SELECTED_PROVINCE_LINE_COLOR));
                    invalidate();
                    //暴露到Activity中的接口，把省的名字传过去
                    onProvinceClickListener.onChose(provinceName);
                    break;
                }
            }
        }
